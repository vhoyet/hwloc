...
hwloc_look_bgq(struct hwloc_backend *backend, struct hwloc_disc_status *dstatus)
{
  ...
  if (hwloc_filter_check_keep_object_type(topology, HWLOC_OBJ_L2CACHE)) {
    ...
    hwloc__insert_object_by_cpuset(topology, NULL, obj, "bgq:l2");
  }
  ...
  for(i=0; i<HWLOC_BGQ_CORES; i++) {
    ...
    if (hwloc_filter_check_keep_object_type(topology, HWLOC_OBJ_L1CACHE)) {
      ...
      hwloc__insert_object_by_cpuset(topology, NULL, obj, "bgq:l1d");
    }
    /* L1i */
    if (hwloc_filter_check_keep_object_type(topology, HWLOC_OBJ_L1ICACHE)) {
      ...
      hwloc__insert_object_by_cpuset(topology, NULL, obj, "bgq:l1i");
    }
