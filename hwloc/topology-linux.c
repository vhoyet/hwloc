				    ...
				    hwloc_bitmap_t cpuset)
{
  ...
  hwloc_debug_2args_bitmap("cache (%s) depth %u has cpuset %s\n",
			   ...
			   level, c->cpuset);
  hwloc__insert_object_by_cpuset(topology, NULL, c, "linux:devicetree:cache");
}
			 ...
			 struct hwloc_linux_backend_data_s *data)
{
  ...
  while (NULL != (dirent = readdir(dt))) {
	...
	if (hwloc_filter_check_keep_object_type(topology, HWLOC_OBJ_CORE)) {
	  ...
	  hwloc__insert_object_by_cpuset(topology, NULL, core, "linux:devicetree:core");
	}
			    ...
			    unsigned *failednodes)
{
  ...
  if (ddr && knl_hwdata->mcdram_cache_size > 0) {
    ...
    if (mscache_as_l3) {
      ...
      hwloc__insert_object_by_cpuset(topology, NULL, cache, "linux:knl:memcache:l3");
    } else {
